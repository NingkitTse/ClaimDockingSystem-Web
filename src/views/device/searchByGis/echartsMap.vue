<template>
  <div class="echarts-map">
    <div id="map-container" ref="myEchart" :style="{ height: '800px', width: '100%' }" />
  </div>
</template>
...
import { getDeptInfo } from '@/api/dict'
let vm = {}
export default {
  data() {
    return {
  ...
  mounted() {
    vm = this
    const topPostcode = 430000
    this.mapContainer = document.getElementById('map-container')

    // 如果是从table界面过来，加上上一次的选项
    const currentOrgNoObjStr = sessionStorage.getItem('current_org_no')
    if (currentOrgNoObjStr) {
      const obj = JSON.parse(currentOrgNoObjStr)
      if (obj && obj.postcode) {
  ...
  methods: {
    ...
    convertData(features) {
      ...
      for (var i = 0; i < features.length; i++) {
        ...
        const postcode = (markdata[i].postcode = feature.id || i + 1)
        const child = this.orgMap.get(
          this.getQualifiedPostcode(this.currentOrg, postcode)
        )
        const enable = (markdata[i].enable = child ? !!child.enableFlag : false)
        ...
        toolbox: {
          ...
          feature: {
            ...
            myTool2: {
              ...
              icon:
                'path://M789.12 818.016s-5.632 14.496-13.12 0c0 0-83.808-252.896-288.064-189.216v78.496s-3.264 46.144-43.52 16.096l-200.832-173.376s-42.592-23.168 2.592-54.88l202.976-174.4s30.528-21.76 37.856 13.92l0.224 84.704c0 0.064 380.704 18.304 301.888 398.656z', // 图标
              onclick: function() {
